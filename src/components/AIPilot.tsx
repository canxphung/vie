...
export default function AIPilot({ language, currentProvinceId, onAddComboToCart }: AIPilotProps) {
  ...
  return (
    <div className="w-full bg-[#F5F2ED] text-[#4A4A35] py-16 px-4 shadow-inner border-y border-[#E6E2D3]">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row gap-10 items-stretch">
          
          {/* Inputs Section */}
          <div className="w-full lg:w-5/12 bg-[#FDFCF8] p-6 lg:p-8 rounded-3xl border border-[#E6E2D3] shadow-xl flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-4">
                <div className="p-2 bg-[#8C7A5B] rounded-lg text-white shadow-md">
                  <Brain className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="text-xl font-serif font-bold tracking-tight text-[#4A4A35] flex items-center gap-1.5">
                    AI Trip Co-Pilot 
                    <span className="text-[10px] font-sans font-bold tracking-widest bg-[#E3B04B] text-white px-1.5 py-0.5 rounded uppercase">Lite</span>
                  </h3>
                  <p className="text-[11px] text-[#4A4A35]/70 font-medium">
              ...
              {/* Slider for Budget */}
              <div className="mb-5 bg-[#F9F7F2] p-4 rounded-xl border border-[#E6E2D3]">
                <div className="flex justify-between items-center text-xs text-[#4A4A35]/80 font-bold mb-1">
                  <span>{isVi ? 'Ngân sách thiết kế' : 'Target Outlay'}</span>
                  <span className="text-[#8C7A5B] font-mono font-bold text-sm">
                ...
                <input 
                  ...
                  className="w-full accent-[#8C7A5B] bg-[#E6E2D3] h-1.5 rounded-lg cursor-pointer"
                />
                <div className="flex justify-between text-[10px] text-[#4A4A35]/50 mt-1 uppercase font-semibold">
                  <span>1.5TR</span>
                  <span>6TR</span>
                  <span>12TR</span>
                </div>
              ...
              {/* Province select */}
              <div className="mb-5">
                <label className="block text-xs font-bold text-[#4A4A35] mb-1">{isVi ? 'Tỉnh thành điểm đến' : 'Target Province Focus'}</label>
                <select
                  value={province}
                  onChange={(e) => setProvince(e.target.value)}
                  className="w-full text-xs font-bold bg-[#FDFCF8] text-[#4A4A35] border border-[#E6E2D3] rounded-xl p-3 focus:border-[#8C7A5B] outline-none"
                >
              ...
              <div className="mb-5">
                <label className="block text-xs font-bold text-[#4A4A35] mb-1">{isVi ? 'Sở thích & Yêu cầu đặc biệt' : 'Special Preferences'}</label>
                <textarea
                  value={customPrompt}
                  ...
                  rows={3}
                  className="w-full text-xs bg-[#FDFCF8] text-[#4A4A35] border border-[#E6E2D3] rounded-xl p-3 focus:outline-none focus:border-[#8C7A5B] placeholder:text-[#8C7A5B]/40"
                />
              </div>

              {/* Preset suggestion chips */}
              <div className="mb-6">
                <p className="text-[10px] text-[#8C7A5B] font-bold uppercase mb-2">{isVi ? 'Gợi ý nhanh cho bạn' : 'Quick Prompt Presets'}</p>
                <div className="flex flex-col gap-2">
                  {presetSuggestions.map((s, idx) => (
                    <button
                      key={idx}
                      onClick={() => handleSuggestClick(s)}
                      className="text-left text-[11px] bg-[#F9F7F2] hover:bg-[#F5F2ED] transition border border-[#E6E2D3] px-3 py-2.5 rounded-xl text-[#4A4A35] hover:text-[#8C7A5B] line-clamp-1"
                    >
                      💡 {s}
                    </button>
                  ))}
            ...
            <button
              ...
              className="w-full bg-[#8C7A5B] hover:bg-[#5A5A40] text-white font-serif font-bold py-3.5 rounded-2xl transition shadow-lg text-xs md:text-sm tracking-wide flex items-center justify-center gap-2 mt-4 cursor-pointer"
              disabled={loading}
            >
          ...
          <div className="w-full lg:w-7/12 bg-[#FDFCF8] p-6 lg:p-8 rounded-3xl border border-[#E6E2D3] shadow-xl relative min-h-[460px] flex flex-col justify-between text-[#4A4A35]">
            {loading ? (
              <div className="absolute inset-0 bg-[#F5F2ED]/95 backdrop-blur-xs flex flex-col items-center justify-center text-center p-6 rounded-3xl z-30">
                <div className="relative">
                  <div className="w-16 h-16 border-4 border-[#8C7A5B] border-t-transparent rounded-full animate-spin"></div>
                  <Brain className="w-6 h-6 text-[#8C7A5B] absolute inset-0 m-auto animate-pulse" />
                </div>
                <h4 className="text-base font-serif font-bold text-[#4A4A35] mt-6 animate-pulse">{isVi ? 'Gemini AI đang suy nghĩ...' : 'Gemini AI reasoning...'}</h4>
                <p className="text-xs text-[#4A4A35]/80 mt-3 max-w-sm px-4 whitespace-pre-wrap leading-relaxed font-mono">
                  {loadingStep}
                </p>
              </div>
            ) : null}
            ...
            {itinerary ? (
              <div className="flex flex-col justify-between h-full">
                <div>
                  <div className="flex flex-wrap justify-between items-start gap-2 border-b border-[#E6E2D3] pb-4 mb-4">
                    <div>
                      <span className="text-[9px] font-sans font-bold uppercase tracking-widest text-[#8C7A5B] border border-[#8C7A5B]/20 bg-[#F5F2ED] px-2.5 py-1 rounded-full">
                        {isVi ? 'Lịch trình thiết kế riêng' : 'Custom Tailored Itinerary'}
                      </span>
                      <h4 className="text-xl font-serif font-bold tracking-tight text-[#4A4A35] mt-2">{itinerary.itineraryTitle}</h4>
                    </div>
                  ...
                  <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-stone-200">
                    {itinerary.days?.map((day: any, dIdx: number) => (
                      <div key={dIdx} className="bg-[#F9F7F2] p-4 border border-[#E6E2D3] rounded-2xl">
                        <h5 className="text-xs font-serif font-bold text-[#8C7A5B] border-b border-[#E6E2D3] pb-2 mb-3 flex items-center gap-1.5 uppercase tracking-wider">
                        ...
                        <div className="space-y-3">
                          {day.activities?.map((act: any, aIdx: number) => (
                            <div key={aIdx} className="flex gap-3 text-xs">
                              <span className="font-mono text-[#8C7A5B] bg-[#F5F2ED] px-2 py-0.5 rounded h-fit shrink-0 font-bold border border-[#E6E2D3]">
                                {act.time}
                              </span>
                              <div>
                                <h6 className="font-serif font-bold text-[#4A4A35] text-xs">{act.attractionName}</h6>
                                <p className="text-[#4A4A35]/80 text-[11px] mt-0.5 leading-relaxed">{act.description}</p>
                                <p className="text-[#8C7A5B] text-[10px] font-bold font-mono mt-1">
                  ...
                  {/* Smart saving tips */}
                  <div className="bg-[#F9F7F2] border border-[#E6E2D3] p-4 rounded-2xl mt-4">
                    <h5 className="text-[10px] font-bold text-[#8C7A5B] uppercase tracking-wider mb-2 flex items-center gap-1.5 font-serif">
                       <HelpCircle className="w-3.5 h-3.5" />
                      {isVi ? 'Khuyến nghị mộc mạc từ AI' : 'Expert Local Insights'}
                    </h5>
                    <ul className="text-[11px] text-[#4A4A35]/85 space-y-1.5 list-disc pl-4 leading-relaxed">
                ...
                <div className="mt-6 pt-4 border-t border-[#E6E2D3] flex flex-wrap justify-between items-center gap-4">
                  <div>
                    <p className="text-[10px] text-[#4A4A35]/60 font-bold uppercase">{isVi ? 'ƯỚC TÍNH TRỌN GÓI COMBO' : 'OPTIMIZED BUNDLE PRICE'}</p>
                    <p className="text-xl font-bold font-mono text-[#8C7A5B]">
              ...
              <div className="flex flex-col items-center justify-center text-center h-full max-w-sm mx-auto">
                <Brain className="w-12 h-12 text-[#8C7A5B]/40 animate-pulse mb-4" />
                <h4 className="text-base font-serif font-bold text-[#4A4A35]">{isVi ? 'Tìm kiếm Giải pháp Itinerary Tối ưu của bạn' : 'Unlock Your Heritage Solution'}</h4>
                <p className="text-xs text-[#4A4A35]/70 mt-2 leading-relaxed">
                  {isVi 
